...
async function main() {
  ...
  const report = [
    ...
    `## Saved Product Pilot`,
    funnelTable([
      ["Products created", count("product_created"), null],
      ["Saved products selected", count("saved_product_selected"), count("product_created")],
      ["References reused", count("reference_reused"), count("saved_product_selected")],
      ["Product histories viewed", count("product_history_viewed"), count("product_created")],
      ["New reference versions promoted", count("reference_version_promoted"), count("product_created")],
    ]),
    ``,
    `## Acquisition`,
