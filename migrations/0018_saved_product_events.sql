...
insert into product_events_next (
  id, idempotency_key, event_name, event_source, anonymous_session_id, analysis_id,
  occurred_at, page_path, referrer_domain, utm_source, utm_medium, utm_campaign,
  utm_content, utm_term, locale, device_class, properties_json, created_at
)
select
  id, idempotency_key, event_name, event_source, anonymous_session_id, analysis_id,
  occurred_at, page_path, referrer_domain, utm_source, utm_medium, utm_campaign,
...
create index idx_product_events_session_time on product_events (anonymous_session_id, occurred_at);
create index idx_product_events_analysis_time on product_events (analysis_id, occurred_at);
create index idx_product_events_name_time on product_events (event_name, occurred_at);
create index idx_product_events_campaign_time on product_events (utm_campaign, occurred_at);
