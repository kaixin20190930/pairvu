...
export async function runRealAnalysis(
  db: D1Database,
  assets: R2Bucket,
  input: Omit<AnalysisCreateInput, "analysisId"> & { analysisId?: string },
  ...
  if (!created) {
    const existing = input.idempotencyKey && input.anonymousSessionId
      ? await getAnalysisByIdempotencyKey(db, input.anonymousSessionId, input.idempotencyKey)
  ...
  try {
    ...
    const engine = new M0QAEngine(provider, new M0RiskPolicy());
    const analysis = await buildAnalysisInput(db, assets, analysisInput);
    const result = await engine.analyze(analysis);

    await persistCompletedAnalysis(db, result, analysisInput);
    await recordAnalysisEventSafely(db, {
...
export async function saveAnalysisFeedback(db: D1Database, input: AnalysisFeedbackInput): Promise<PersistedAnalysisResult | null> {
  ...
  if (!existing || existing.anonymousSessionId !== input.anonymousSessionId) {
    return null;
  }
...
function buildOpenAIProvider(): VisionProvider {
