...
export const checkPageContents: readonly CheckPageContent[] = [
  ...
  },
  {
    ...
    founderApprovedAt: "2026-08-04",
    audience:
      "Brand managers, product photographers, ecommerce operators, catalog teams, creative agencies, and production reviewers who must verify that an AI-generated or edited product image contains the approved container, closure, functional parts, included accessories, and product-set relationships before publishing.",
    directAnswer:
      "To check product components in an AI image, first define the approved visible set: the primary product or container, attached functional hardware, closures, and every separate accessory that the image is expected to represent. Compare the same areas in the candidate and classify each item as present, missing, substituted, extra, hidden, or outside the frame. PASS only when all required observable components correspond and no unapproved component appears. Use REVIEW when a crop, overlap, viewpoint, or merchandising ambiguity prevents confirmation. Use FAIL when a critical identity-bearing set claim is visibly wrong or a policy-approved required component difference is confirmed.",
    scopeDistinction:
      "A component is not the same as a primary product count, decorative prop, package shape, or printed claim. A wick trimmer beside a candle may be an included accessory; a second candle jar changes primary product count; a background vase is scene decor; a fitted lid contributes to package construction; and the words CEDAR CANDLE SET are visible identity evidence. Pairvu reports what the pixels support but cannot infer what the customer will receive, whether an accessory was intentionally added, or whether a hidden item exists outside the supplied frame.",
    deck:
      "AI product images can quietly remove a trigger, replace a cap, add an applicator, omit a set accessory, or crop away the only area that proves a component is present. Product Components QA separates confirmed set changes from harmless scene edits and missing visual coverage so teams can correct the image without turning uncertainty into a false accusation.",
    dimensions: [
      {
        title: "Primary container and product body",
        definition:
          "The central sellable object or housing that carries the product identity, such as a jar, bottle, tube, carton, pouch, can, dispenser body, or appliance shell. It anchors all component relationships and must remain distinct from duplicate primary units.",
        example:
          "The EMBERNOOK candle jar remains one complete primary product in every case even when a wick trimmer disappears or a snuffer appears beside it.",
      },
      {
        title: "Closures and dispensing hardware",
        definition:
          "Caps, lids, pumps, triggers, droppers, sprayers, valves, flip tops, seals, zippers, and other visible hardware that closes, protects, opens, or dispenses the product. Their presence and attachment may define practical package identity.",
        example:
          "A cleaner bottle with its trigger removed is not equivalent to the approved spray product even when the bottle body and label remain unchanged.",
      },
      {
        title: "Separate included accessories",
        definition:
          "Discrete tools, applicators, scoops, brushes, cables, inserts, trimmers, snuffers, or other items presented beside the primary product as part of the approved set. Their complete boundaries should be visible when inclusion matters.",
        example:
          "The long brass wick trimmer is an approved candle-set accessory; an added brass snuffer changes the visible set and requires intent confirmation.",
      },
      {
        title: "Component identity and substitution",
        definition:
          "Whether the visible part is the same kind, design, material family, and functional role as the approved component rather than a plausible-looking replacement generated by the model.",
        example:
          "A white dropper that becomes a pump, or a brass wick trimmer that becomes ordinary scissors, is a substitution even if the total number of objects stays constant.",
      },
      {
        title: "Set membership and object relationship",
        definition:
          "How a part relates to the primary product: attached hardware, detached but included accessory, secondary sellable unit, packaging element, or unrelated scene prop. Position alone does not establish membership, so label language and visual grouping also matter.",
        example:
          "A snuffer placed beside a CEDAR CANDLE SET plausibly reads as included product hardware, while a distant plant in the background does not.",
      },
      {
        title: "Component coverage and separability",
        definition:
          "Whether the candidate shows enough of each required area to distinguish presence, absence, overlap, crop, and occlusion. The checker needs visible boundaries or corresponding attachment points rather than assumptions about what should exist.",
        example:
          "A tight crop can verify the complete candle jar and lid while leaving the right-side wick trimmer entirely outside the frame, requiring REVIEW rather than a missing-component claim.",
      },
    ],
    decisionRules: [
      {
        condition: "Required attached component",
        pass: "The approved closure, dispenser, or functional hardware is visible in the corresponding candidate location with the same recognizable role and attachment.",
        review: "The attachment point is turned away, hidden, cropped, overlapped, or too small to establish whether the required hardware is still present.",
        fail: "A required attached component is clearly absent or replaced and the approved policy identifies that confirmed difference as publication-critical.",
      },
      {
        condition: "Separate included accessory",
        pass: "Every approved separate accessory is fully observable and corresponds in type, count, and relationship to the same product set.",
        review: "An accessory is outside the candidate frame, ambiguously occluded, or visible but the checker cannot determine whether its inclusion was intentionally changed.",
        fail: "The accessory difference is confirmed together with critical identity evidence, or an approved rule explicitly marks that required set omission as a failure.",
      },
      {
        condition: "Extra product-related object",
        pass: "No unapproved included-looking part appears; any new visible object is clearly unrelated scene decor and does not imply another product entitlement.",
        review: "A new applicator, tool, attachment, or accessory plausibly belongs to the offer but user intent cannot be established from the images alone.",
        fail: "The extra object contradicts an approval-critical set definition or a policy-confirmed offer and cannot reasonably be treated as decor.",
      },
      {
        condition: "Component substitution",
        pass: "The same component type, material family, functional design, and visible attachment remain consistent despite harmless perspective or lighting changes.",
        review: "The candidate suggests a replacement component but resolution, angle, or overlap prevents a reliable identity comparison.",
        fail: "A clearly observable approved component has been replaced with a different functional part and the substitution violates the accepted product identity.",
      },
      {
        condition: "Scene, position, and recomposition",
        pass: "Components move within the frame or enter a new environment while the complete approved set stays observable and unchanged.",
        review: "Recomposition places a required item behind the primary product, beyond an edge, or among props so membership and presence are uncertain.",
        fail: "The scene edit also creates a confirmed component or identity difference rather than only changing presentation variables.",
      },
      {
        condition: "Crop and occlusion",
        pass: "The crop still includes every required component boundary and the corresponding relationship between each item and the primary product.",
        review: "The candidate excludes or covers the exact region needed to verify one or more approved parts, so absence cannot be proven.",
        fail: "Both images provide complete corresponding coverage and directly establish that an approval-critical required part has been removed.",
      },
      {
        condition: "Set wording and visible identity",
        pass: "Set-related names, counts, and included-item wording remain readable and consistent with the visible component configuration.",
        review: "Set wording is unreadable or its commercial meaning conflicts ambiguously with the visible grouping and needs owner confirmation.",
        fail: "Readable identity wording changes, such as CEDAR CANDLE SET becoming CEDAR CANDLE, and the candidate no longer represents the approved set.",
      },
    ],
    evidence: [
      {
        href: "/examples/candle-set-component-removed",
        title: "The wick trimmer and SET identity were removed",
        role: "product_change",
        decision: "FAIL",
        original: "/examples/candle-components/original.jpg",
        candidate: "/examples/candle-components/missing-trimmer.jpg",
        alt: "Approved candle set with wick trimmer compared with a candle missing the trimmer and SET wording",
        observation:
          "The approved image contains one dark-green candle jar with fitted brass lid and a complete separate brass wick trimmer. The candidate preserves the jar, lid, circular flame logo, 220 g value, color, and silhouette but omits the trimmer and changes CEDAR CANDLE SET to CEDAR CANDLE.",
        whyThisDecision:
          "Pairvu recorded both a missing-component finding and a critical visible-text finding. The readable set-identity change supplies the current FAIL path. This evidence does not claim that every component-only omission receives FAIL under M0.",
        nextAction:
          "Restore both the approved accessory and exact set wording, then compare the corrected final image against the same reference.",
      },
      {
        href: "/examples/candle-set-scene-change",
        title: "A new scene preserved the complete candle set",
        role: "hard_negative",
        decision: "PASS",
        original: "/examples/candle-components/original.jpg",
        candidate: "/examples/candle-components/scene-change.jpg",
        alt: "Same candle jar, lid, and wick trimmer shown in studio and textured stone scenes",
        observation:
          "The candidate changes background, apparent scale, and spacing while retaining one candle jar, fitted lid, complete wick trimmer, exact label wording, logo, 220 g value, main colors, and package construction.",
        whyThisDecision:
          "Component membership is semantic, not pixel-based. Every required part remains directly observable, so the scene recomposition is a valid hard negative and should not create a missing or extra component alarm.",
        nextAction:
          "Accept the component result and review the new composition separately against the creative brief if placement requirements matter.",
      },
      {
        href: "/examples/candle-accessory-outside-crop",
        title: "The candidate crop excluded the approved accessory area",
        role: "observability",
        decision: "REVIEW",
        original: "/examples/candle-components/original.jpg",
        candidate: "/examples/candle-components/trimmer-crop.jpg",
        alt: "Complete candle set compared with a tight crop that excludes the wick trimmer",
        observation:
          "The candidate fully shows the candle jar, fitted lid, flame logo, CEDAR CANDLE SET wording, 220 g value, color, and jar silhouette. It does not include any pixels from the right-side region where the approved wick trimmer appears.",
        whyThisDecision:
          "No candidate evidence proves presence or removal of the trimmer. Pairvu correctly reports insufficient component observability rather than inventing a mismatch from missing coverage.",
        nextAction:
          "Request a wider export that includes the complete approved product set before granting component approval.",
      },
      {
        href: "/examples/extra-candle-accessory",
        title: "A brass candle snuffer was added to the set",
        role: "product_change",
        decision: "REVIEW",
        original: "/examples/candle-components/original.jpg",
        candidate: "/examples/candle-components/extra-snuffer.jpg",
        alt: "Approved candle jar and wick trimmer compared with a candidate that adds a brass candle snuffer",
        observation:
          "The candidate keeps the approved jar, fitted lid, wick trimmer, logo, wording, 220 g value, color, and package shape, while adding one separate bell-shaped brass snuffer on the left. The overall scene is also recomposed.",
        whyThisDecision:
          "The extra accessory is clearly observable, but the checker cannot infer whether the merchandising brief intentionally expanded the set. REVIEW prevents automatic approval without overstating the addition as a policy-confirmed failure.",
        nextAction:
          "Remove the snuffer if it is unapproved, or use a new approved reference that explicitly represents the expanded candle-care set.",
      },
    ],
    diagnosticQuestions: [
      {
        question: "What exactly belongs to the approved visible set?",
        reason:
          "List the primary body, closures, attached hardware, and separate accessories before comparing. If the baseline is vague, reviewers will confuse props with products or overlook expected parts.",
      },
      {
        question: "Is the object a component, another product unit, packaging, or decor?",
        reason:
          "A separate applicator may be an included accessory, a second jar changes primary count, a gift box changes packaging, and a plant changes only the scene. Each requires a different decision path.",
      },
      {
        question: "Can the complete boundary and attachment point be seen?",
        reason:
          "Complete boundaries help distinguish a removed part from a part hidden behind the main product, while attachment evidence confirms whether a closure or dispenser is actually installed.",
      },
      {
        question: "Does readable wording define the set or included item?",
        reason:
          "Names such as SET, KIT, WITH APPLICATOR, or TWO-PIECE can supply identity evidence that changes the severity of an otherwise ambiguous object difference.",
      },
      {
        question: "Did the new scene merely move the same approved objects?",
        reason:
          "Background, scale, spacing, and perspective may change without altering set membership. Confirm inventory before treating composition drift as a component problem.",
      },
      {
        question: "Would a wider or clearer candidate resolve the uncertainty?",
        reason:
          "When crop, overlap, or occlusion is the only problem, the correct action is to obtain more evidence rather than regenerate a part that may already exist outside the frame.",
      },
    ],
    failureModes: [
      {
        title: "Accessory deletion during cleanup",
        mechanism:
          "Generative cleanup interprets a small separate tool, scoop, applicator, or cable as clutter and reconstructs the background over it.",
        consequence:
          "The candidate may advertise an incomplete set even though the primary package, logo, and text appear polished and accurate.",
      },
      {
        title: "Plausible but wrong component substitution",
        mechanism:
          "The model preserves a component category but redraws it as another familiar form, such as changing a dropper into a pump or a specialist trimmer into ordinary scissors.",
        consequence:
          "The image can misrepresent use, compatibility, included hardware, or package identity while passing a coarse object-count check.",
      },
      {
        title: "Decor promoted into an included accessory",
        mechanism:
          "A generated tool or prop is placed close to the primary package, shares its material and styling, and visually reads as part of the offer.",
        consequence:
          "Customers may infer an entitlement that the listing does not include, creating support, return, or advertising-risk problems.",
      },
      {
        title: "Crop mistaken for confirmed removal",
        mechanism:
          "A candidate detail crop excludes the approved accessory area while leaving the primary product complete enough to look comparable.",
        consequence:
          "Calling the component missing would produce a false alarm; approving the full set would claim evidence that the candidate does not provide.",
      },
      {
        title: "Overlap hides set membership",
        mechanism:
          "An accessory moves behind a jar, carton, hand, ribbon, or another product so only an ambiguous fragment or no boundary remains visible.",
        consequence:
          "Reviewers cannot determine whether the approved item is present, substituted, or absent without another view or less obstructed export.",
      },
      {
        title: "Set wording and visible objects drift apart",
        mechanism:
          "The image generator edits words such as SET, KIT, or WITH BRUSH independently from the objects shown beside the package.",
        consequence:
          "Text and visual inventory communicate conflicting offers, which can be more commercially serious than either isolated mismatch.",
      },
    ],
    workflow: [
      {
        title: "Write the approved component inventory",
        detail:
          "Name the primary product body, closure, attached hardware, and each separate included accessory. Record which items must be visible in this channel image and which may legitimately remain hidden.",
      },
      {
        title: "Mark component-bearing regions",
        detail:
          "Locate attachment points and expected accessory zones in the approved image. Note whether each item is fully visible, partly occluded, detached, or represented by set wording.",
      },
      {
        title: "Confirm corresponding candidate coverage",
        detail:
          "Before judging presence, verify that the candidate includes the same region or provides another clear view of the same component relationship. Missing pixels are not missing products.",
      },
      {
        title: "Classify every apparent object difference",
        detail:
          "Label each as required component, replacement, extra accessory, duplicate primary unit, packaging element, or unrelated prop. Do not collapse all additional objects into one generic count finding.",
      },
      {
        title: "Cross-check set identity text",
        detail:
          "Read product names, SET or KIT language, included-item claims, and printed counts. Resolve conflicts between what the label promises and what the image visibly contains.",
      },
      {
        title: "Apply the evidence-aware verdict",
        detail:
          "PASS a complete observable match, REVIEW uncertain coverage or an intent-dependent addition, and FAIL only when confirmed critical evidence and approved policy support that decision.",
      },
      {
        title: "Resolve and rerun the final export",
        detail:
          "Restore or remove the correct object, request a wider view, or select a reference for the intentional set. Compare the corrected final image again and capture reviewer feedback.",
      },
    ],
    limitations: [
      "Pairvu evaluates visible image evidence and cannot prove what will be packed, shipped, assembled, or delivered to a customer.",
      "A PASS applies only to components observable in corresponding regions; backs, interiors, inserts, hidden attachments, and out-of-frame accessories remain unverified.",
      "The checker cannot infer whether an extra tool is intentional, promotional, optional, sold separately, or merely a scene prop without product-owner context.",
      "Very small, transparent, reflective, overlapping, partially assembled, or low-contrast components may require REVIEW even when the product is unchanged.",
      "One front image cannot certify mechanical function, fit, material composition, electrical compatibility, safety, durability, or regulatory compliance of a component.",
      "Product Components QA does not replace a bill of materials, packing list, product profile, assembly drawing, channel specification, or physical sample inspection.",
      "The current M0 policy may REVIEW a confirmed component-only addition or omission when user intent is unknown; it must not be advertised as an automatic FAIL rule.",
      "A component match does not certify logo, wording, printed quantity, primary product count, exact color, package geometry, claims, or marketplace approval.",
    ],
    faq: [
      {
        question: "What counts as a product component in an AI image?",
        answer:
          "A component is a visible part of the approved product or set, including closures, dispensers, attached hardware, and separate included accessories. Primary sellable units, outer packaging, and unrelated scene props should be classified separately.",
      },
      {
        question: "Does a missing component always make the result FAIL?",
        answer:
          "No. Under the current M0 behavior, a component-only difference may receive REVIEW when intent is unknown. The candle FAIL example also contains a critical visible-text change from CEDAR CANDLE SET to CEDAR CANDLE.",
      },
      {
        question: "Why is an accessory outside the crop not reported as missing?",
        answer:
          "Because the candidate supplies no corresponding pixels for that accessory area. The component may still exist outside the frame. REVIEW records the evidence gap without inventing removal.",
      },
      {
        question: "Can a background prop create a component finding?",
        answer:
          "A clearly unrelated plant or furniture item should not. A tool placed beside the product in matching materials can plausibly look included, so Pairvu may request review when set membership is ambiguous.",
      },
      {
        question: "How should an intentional extra accessory be approved?",
        answer:
          "Use an approved reference that shows the expanded set or confirm the addition through the product-owner workflow. Then rerun the candidate against that reference instead of treating the original single-set image as the baseline.",
      },
      {
        question: "Can Pairvu verify a component that is hidden behind the product?",
        answer:
          "No. It can report that coverage is insufficient, but it cannot confirm presence or absence from hidden space. Supply another angle or a candidate with all required parts visible.",
      },
      {
