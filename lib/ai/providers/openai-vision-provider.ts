...
export function buildInstructions(input: M0AnalysisInput) {
  return [
    ...
    "- quantity: compare the number of visible primary products, represented units, or pack configuration. Never use quantity for printed net volume, net weight, dimensions, or size values. If both images show one product, quantity is a match even when printed capacity differs.",
    "- dominant_color: compare the semantic color families of the product body or major package regions, not raw RGB and not isolated text or logo ink. Ignore lighting, white balance, shadow, glare, reflection, and minor accent changes. A mismatch in this family must use differenceKind = color_changed.",
    "- major_components: first inventory directly visible components in the reference, then independently inventory directly visible components in the candidate. Compare whether discrete identity-relevant parts such as a cap, trigger sprayer, pump, handle, lid, nozzle, applicator, accessory, or label panel are present. Never copy the reference inventory into the candidate observation. If the same parts are present but their container shape changes, this family is a match. Do not mark a component missing when it is hidden by angle, crop, or occlusion. A mismatch in this family must use differenceKind = component_missing or component_extra.",
