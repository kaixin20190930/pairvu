...
The label-only rerun confirmed a provider observability failure rather than a contaminated fixture. Prompt `m0-real-mvp-008` added explicit coverage-scope rules and `m0-qa-engine-004` required sufficient observability for mismatches. The production smoke then exposed the symmetric issue: quantity was marked match even though its own evidence said the candidate product was not visible and count had been assumed from the reference. Prompt `m0-real-mvp-009` now forbids inferring complete primary-product count from a label detail, and `m0-qa-engine-005` applies the sufficient-observability invariant to both match and mismatch. Provider status and evidence remain preserved in raw audit data. `M0RiskPolicy` remains unchanged. Production acceptance now requires REVIEW with logo and visible text verified, and product count, main color, major components, and package shape marked not observable.
